...
We can execute our main function - `collect_qb_stats` - from within R via the `reticulate` package. 
...
qb_df = collect_qb_stats()
```

```{r include=FALSE}
qb_df = read_csv(file.path(getwd(), 'data', 'quarterback_data.csv')) 
```
...
qb_df %>%
  head(10) %>%
  gt() %>%
  tab_header(title = gt::md('**Data Sample of QB Passing Stats**')) %>%
  cols_align(
  align = "center",
  columns = everything()) %>%
  cols_width(
...
Let's add our indicator for success below. 
...
And now we have all the data to get started.
...
```
Let’s examine how our estimates for each player change as the season unfolds. I’ve plotted this process by storing the Probability Density Functions (PDFs) following each week (a total of 16 games) during the regular season. I've also used a custom plotting theme and colors, which can be found below in the [Appendix](#appendix).
