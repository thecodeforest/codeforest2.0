...
![](/post/2019-08-29-fantasy-football-kickers/images/header_image.jpg){width=700px height=400px}  
...
With that question in mind, let's load all pertinent libraries. The data can be downloaded directly from the `r emo::ji("tree")` [the codeforest data repo](https://github.com/thecodeforest) `r emo::ji("tree")`. Note the original data comes from Kaggle and can found [here](https://www.kaggle.com/kendallgillies/nflstatistics/version/1?select=Career_Stats_Field_Goal_Kickers.csv).
...
fake_df %>%
  gt() %>%
  tab_header(title = gt::md('**Desired Data Format**')) %>% 
  cols_align(
  align = "center",
  columns = everything()) %>% 
  cols_width(
...
```{r echo = FALSE, message=FALSE, warning=FALSE}
  ...
  select(name, year, fg_made, fg_attempted) %>% 
  gt() %>%
  tab_header(title = gt::md('**Justin Tucker Stats**')) %>% 
  cols_align(
  align = "center",
  columns = everything()) %>% 
  cols_width(
